...
- **Linting and formatting**: No linter is configured in the project. In production, add `ruff` as a dev dependency with a `[tool.ruff]` section in `pyproject.toml` (target Python 3.12, line-length 100, enable pyflakes + pycodestyle + isort rules). Enforce via CI (the `.github/workflows/ci.yml` already includes a `ruff check .` step) and pre-commit hooks. This catches import ordering, unused variables, and style inconsistencies before they reach code review.
- **Feature contract**: The API validates input via Pydantic (`PredictionRequest`) and the pipeline uses `src/data/validation.py` for schema checks. In production, unify both into a single Pydantic-based feature contract (`src/data/feature_contract.py`) that defines expected feature names, types, and valid ranges as one source of truth. Both the API and the training pipeline import the same contract, so schema changes are enforced in one place. This also enables auto-generating API docs and data dictionaries from the contract.
- **Model registry**: Currently the best model is saved as `artifacts/best_model.pkl` with metadata in `model_metadata.json`. In production, register each trained model in MLflow's Model Registry via `mlflow.register_model()` with stage transitions (Staging → Production). This enables: (1) version comparison across training runs, (2) rollback to a previous model version with a single API call, (3) approval workflows before production promotion, and (4) a single source of truth for which model is currently serving.
